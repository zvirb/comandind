#!/bin/sh
# This is a generic wrapper script for service entrypoints.
# It performs two main functions:
# 1. Certificate Isolation: Copies service-specific certificates from a shared
#    volume to a private, in-container location.
# 2. Privilege Dropping: If the RUN_AS_USER environment variable is set, it
#    executes the final command as that user, adhering to the principle of
#    least privilege.

set -e

# --- Configuration ---
# SERVICE_NAME: The name of the service, used to find its certs in the shared volume.
# RUN_AS_USER: Optional. The user:group to run the final command as (e.g., "nobody:nogroup").
SERVICE_NAME="${SERVICE_NAME:-default}"
RUN_AS_USER="${RUN_AS_USER:-}"
SHARED_CERTS_DIR="/tmp/certs-volume/${SERVICE_NAME}"
PRIVATE_CERTS_DIR="/app/certs/${SERVICE_NAME}"

# --- Certificate Isolation ---
# Use app directory instead of /etc/certs to avoid permission issues
if [ -d "${SHARED_CERTS_DIR}" ]; then
    echo "Wrapper: [${SERVICE_NAME}] Isolating certificates..."
    mkdir -p "${PRIVATE_CERTS_DIR}"
    # Use cp with relaxed permissions to avoid failures
    if cp -r "${SHARED_CERTS_DIR}/." "${PRIVATE_CERTS_DIR}/" 2>/dev/null; then
        echo "Wrapper: [${SERVICE_NAME}] Certificates copied to ${PRIVATE_CERTS_DIR}."
        # If we are going to drop privileges, ensure the new user owns the certs.
        if [ -n "$RUN_AS_USER" ]; then
            echo "Wrapper: [${SERVICE_NAME}] Setting ownership of ${PRIVATE_CERTS_DIR} to ${RUN_AS_USER}."
            chown -R "$RUN_AS_USER" "${PRIVATE_CERTS_DIR}" 2>/dev/null || true
        fi
    else
        echo "Wrapper: [${SERVICE_NAME}] Warning: Could not copy certificates, continuing without them."
    fi
else
    echo "Wrapper: [${SERVICE_NAME}] No certificates directory found, continuing without certificates."
fi

# --- Handover to Main Process ---
echo "Wrapper: [${SERVICE_NAME}] Handing over to main process..."

# If RUN_AS_USER is set, use su-exec to drop privileges. Otherwise, exec directly.
# This requires the 'su-exec' utility to be installed in the container image.
if [ -n "$RUN_AS_USER" ]; then
    echo "Wrapper: [${SERVICE_NAME}] Executing command as user ${RUN_AS_USER}: $@"
    exec su-exec "$RUN_AS_USER" "$@"
else
    echo "Wrapper: [${SERVICE_NAME}] Executing command as current user: $@"
    exec "$@"
fi