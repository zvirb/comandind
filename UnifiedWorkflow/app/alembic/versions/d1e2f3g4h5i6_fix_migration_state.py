...
def upgrade() -> None:
    """Ensure all required tables exist without conflicts."""
    
    # Check if user_history_summaries table exists, create if not
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    tables = inspector.get_table_names()
    
    if 'user_history_summaries' not in tables:
